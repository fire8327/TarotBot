...
async def invite_friend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать реферальную ссылку пользователю"""
    if update.message:
        user_id = update.message.from_user.id
    ...
    bot_username = "speculora_bot"  # Убедитесь, что это правильный username
    ref_link = f"https://t.me/{bot_username}?start=ref_{user_id}"

    # ✅ Используем текст из словаря TEXTS
    await send_method(
        TEXTS['invite_friend'],
        parse_mode='Markdown',
        reply_markup=main_menu_keyboard()
    ...
    # Дополнительно: кнопки для удобства (опционально)
    keyboard = [
    ...
    await send_method(
        "Или поделись ссылкой через кнопку:",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
🔄 *ГЛАВНЫЕ ИЗМЕНЕНИЯ:*
• 🎁 Новая реферальная система — тестируется новый код (если что-то не так, то обратитесь к разработчику)!
• 📞 Добавлена обратная связь — теперь вы можете напрямую писать разработчику
• ⚡ Упрощённый процесс получения раскладов
• 🔧 Исправлены мелкие ошибки и улучшена стабильность

💡 *ХОТИМ УСЛЫШАТЬ ВАС:*
Будем рады вашим предложениям и идеям через раздел «📞 Обратная связь». 
Самые популярные и интересные предложения обязательно будут добавлены в бота!
...
*P.S. Чувствуете перемены? Карты стали говорить ещё яснее...*"""
    ...
    for user in users:
        try:
            # 🔥 ДОБАВЛЯЕМ КНОПКУ ДЛЯ ПОДТВЕРЖДЕНИЯ ОБНОВЛЕНИЯ
            keyboard = [
                [InlineKeyboardButton("🔄 Подтвердить обновление", url=f"https://t.me/speculora_bot?start=update")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    await update.message.reply_text(
        ...
        f"💡 Пользователи получат кнопку для подтверждения обновления",
        parse_mode='Markdown'
    )
...
async def admin_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input == '🎁 Добавить расклады ВСЕМ':
        ...
        # 🔥 ОБНОВЛЯЕМ ТЕКСТ ПОДТВЕРЖДЕНИЯ (без упоминания раскладов)
        keyboard = [
            ['✅ Да, сделать рассылку'],
            ['❌ Нет, отменить']
        ]
        await update.message.reply_text(
            ...
            "• Пользователи узнают о новых функциях\n"
            "• Рассылка займет несколько минут\n\n"
            "💡 *Для начисления раскладов используйте «🎁 Добавить расклады ВСЕМ»*\n\n"
